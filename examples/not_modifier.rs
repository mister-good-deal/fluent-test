...
fn main() {
    ...
    // 3. Using the ! operator (Rust-specific shorthand)
    !expect(true).to_equal(false);

    // 4. Using the not! macro
    not!(5).to_equal(10);

    // 5. Jest-style property access (the most natural) ✓
    ...
    !expect(value).to_equal(100); // Using ! operator
    not!(value).to_equal(100); // Using macro
    expect(value).not.to_equal(100); // Jest-style property
