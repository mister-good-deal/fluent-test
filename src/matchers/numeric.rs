...
// Implement NumericMatchers for NotExpectation to allow using the Jest-like syntax
impl<T> NumericMatchers<T> for NotExpectation<T>
where
    T: Debug + PartialOrd + Copy + Clone,
{
    fn to_be_greater_than(self, expected: T) {
        let result = self.value > expected;
        let success = if self.negated { !result } else { result };
