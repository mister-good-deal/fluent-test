...
impl<T: AsRef<str> + Debug + Clone> StringExt for NotExpectation<T> {
    fn to_be_empty(self) {
        let value = self.value.as_ref();
        let result = value.is_empty();
        let success = if self.negated { !result } else { result };
...
// Implement StringMatchers for NotExpectation to support Jest-like syntax
impl<T: AsRef<str> + Debug + Clone> StringMatchers for NotExpectation<T> {
    fn to_contain(self, substring: &str) {
        let value = self.value.as_ref();
        let result = value.contains(substring);
        let success = if self.negated { !result } else { result };
    ...
    // Implementation for other string matchers would follow the same pattern
    fn to_match_regex(self, pattern: &str) {
        let value = self.value.as_ref();
        let regex = match Regex::new(pattern) {
            Ok(r) => r,
